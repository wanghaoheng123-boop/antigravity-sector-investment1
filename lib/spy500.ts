/**
 * S&P 500 constituent list with GICS sector labels.
 * Source: S&P 500 index as of 2025. Static reference — update quarterly if needed.
 * Covers all 11 GICS sectors for sector-filter support in EMA ranking.
 */

export interface SpyConstituent {
  ticker: string
  sector: string
}

export const SPY500: SpyConstituent[] = [
  // Information Technology
  { ticker: 'AAPL', sector: 'Information Technology' },
  { ticker: 'MSFT', sector: 'Information Technology' },
  { ticker: 'NVDA', sector: 'Information Technology' },
  { ticker: 'AVGO', sector: 'Information Technology' },
  { ticker: 'ORCL', sector: 'Information Technology' },
  { ticker: 'AMD', sector: 'Information Technology' },
  { ticker: 'AMAT', sector: 'Information Technology' },
  { ticker: 'QCOM', sector: 'Information Technology' },
  { ticker: 'TXN', sector: 'Information Technology' },
  { ticker: 'INTC', sector: 'Information Technology' },
  { ticker: 'MU', sector: 'Information Technology' },
  { ticker: 'LRCX', sector: 'Information Technology' },
  { ticker: 'KLAC', sector: 'Information Technology' },
  { ticker: 'ADI', sector: 'Information Technology' },
  { ticker: 'MCHP', sector: 'Information Technology' },
  { ticker: 'CDNS', sector: 'Information Technology' },
  { ticker: 'SNPS', sector: 'Information Technology' },
  { ticker: 'PANW', sector: 'Information Technology' },
  { ticker: 'CRWD', sector: 'Information Technology' },
  { ticker: 'FTNT', sector: 'Information Technology' },
  { ticker: 'NOW', sector: 'Information Technology' },
  { ticker: 'INTU', sector: 'Information Technology' },
  { ticker: 'ADBE', sector: 'Information Technology' },
  { ticker: 'CRM', sector: 'Information Technology' },
  { ticker: 'IBM', sector: 'Information Technology' },
  { ticker: 'HPQ', sector: 'Information Technology' },
  { ticker: 'DELL', sector: 'Information Technology' },
  { ticker: 'STX', sector: 'Information Technology' },
  { ticker: 'WDC', sector: 'Information Technology' },
  { ticker: 'KEYS', sector: 'Information Technology' },

  // Communication Services
  { ticker: 'META', sector: 'Communication Services' },
  { ticker: 'GOOGL', sector: 'Communication Services' },
  { ticker: 'GOOG', sector: 'Communication Services' },
  { ticker: 'NFLX', sector: 'Communication Services' },
  { ticker: 'DIS', sector: 'Communication Services' },
  { ticker: 'CMCSA', sector: 'Communication Services' },
  { ticker: 'T', sector: 'Communication Services' },
  { ticker: 'VZ', sector: 'Communication Services' },
  { ticker: 'TMUS', sector: 'Communication Services' },
  { ticker: 'CHTR', sector: 'Communication Services' },
  { ticker: 'EA', sector: 'Communication Services' },
  { ticker: 'TTWO', sector: 'Communication Services' },
  { ticker: 'WBD', sector: 'Communication Services' },
  { ticker: 'PARA', sector: 'Communication Services' },
  { ticker: 'OMC', sector: 'Communication Services' },

  // Consumer Discretionary
  { ticker: 'AMZN', sector: 'Consumer Discretionary' },
  { ticker: 'TSLA', sector: 'Consumer Discretionary' },
  { ticker: 'HD', sector: 'Consumer Discretionary' },
  { ticker: 'MCD', sector: 'Consumer Discretionary' },
  { ticker: 'NKE', sector: 'Consumer Discretionary' },
  { ticker: 'LOW', sector: 'Consumer Discretionary' },
  { ticker: 'SBUX', sector: 'Consumer Discretionary' },
  { ticker: 'TJX', sector: 'Consumer Discretionary' },
  { ticker: 'BKNG', sector: 'Consumer Discretionary' },
  { ticker: 'MAR', sector: 'Consumer Discretionary' },
  { ticker: 'HLT', sector: 'Consumer Discretionary' },
  { ticker: 'GM', sector: 'Consumer Discretionary' },
  { ticker: 'F', sector: 'Consumer Discretionary' },
  { ticker: 'ORLY', sector: 'Consumer Discretionary' },
  { ticker: 'AZO', sector: 'Consumer Discretionary' },
  { ticker: 'EBAY', sector: 'Consumer Discretionary' },
  { ticker: 'ETSY', sector: 'Consumer Discretionary' },
  { ticker: 'RCL', sector: 'Consumer Discretionary' },
  { ticker: 'CCL', sector: 'Consumer Discretionary' },
  { ticker: 'YUM', sector: 'Consumer Discretionary' },

  // Consumer Staples
  { ticker: 'WMT', sector: 'Consumer Staples' },
  { ticker: 'PG', sector: 'Consumer Staples' },
  { ticker: 'KO', sector: 'Consumer Staples' },
  { ticker: 'PEP', sector: 'Consumer Staples' },
  { ticker: 'COST', sector: 'Consumer Staples' },
  { ticker: 'MDLZ', sector: 'Consumer Staples' },
  { ticker: 'MO', sector: 'Consumer Staples' },
  { ticker: 'PM', sector: 'Consumer Staples' },
  { ticker: 'CL', sector: 'Consumer Staples' },
  { ticker: 'GIS', sector: 'Consumer Staples' },
  { ticker: 'KHC', sector: 'Consumer Staples' },
  { ticker: 'KR', sector: 'Consumer Staples' },
  { ticker: 'SYY', sector: 'Consumer Staples' },
  { ticker: 'CAG', sector: 'Consumer Staples' },
  { ticker: 'HSY', sector: 'Consumer Staples' },

  // Health Care
  { ticker: 'LLY', sector: 'Health Care' },
  { ticker: 'UNH', sector: 'Health Care' },
  { ticker: 'JNJ', sector: 'Health Care' },
  { ticker: 'ABBV', sector: 'Health Care' },
  { ticker: 'MRK', sector: 'Health Care' },
  { ticker: 'TMO', sector: 'Health Care' },
  { ticker: 'ABT', sector: 'Health Care' },
  { ticker: 'DHR', sector: 'Health Care' },
  { ticker: 'PFE', sector: 'Health Care' },
  { ticker: 'AMGN', sector: 'Health Care' },
  { ticker: 'GILD', sector: 'Health Care' },
  { ticker: 'ISRG', sector: 'Health Care' },
  { ticker: 'SYK', sector: 'Health Care' },
  { ticker: 'VRTX', sector: 'Health Care' },
  { ticker: 'REGN', sector: 'Health Care' },
  { ticker: 'BIIB', sector: 'Health Care' },
  { ticker: 'MDT', sector: 'Health Care' },
  { ticker: 'BSX', sector: 'Health Care' },
  { ticker: 'ELV', sector: 'Health Care' },
  { ticker: 'CI', sector: 'Health Care' },

  // Financials
  { ticker: 'BRK-B', sector: 'Financials' },
  { ticker: 'JPM', sector: 'Financials' },
  { ticker: 'BAC', sector: 'Financials' },
  { ticker: 'WFC', sector: 'Financials' },
  { ticker: 'GS', sector: 'Financials' },
  { ticker: 'MS', sector: 'Financials' },
  { ticker: 'BLK', sector: 'Financials' },
  { ticker: 'C', sector: 'Financials' },
  { ticker: 'SCHW', sector: 'Financials' },
  { ticker: 'AXP', sector: 'Financials' },
  { ticker: 'V', sector: 'Financials' },
  { ticker: 'MA', sector: 'Financials' },
  { ticker: 'PGR', sector: 'Financials' },
  { ticker: 'TRV', sector: 'Financials' },
  { ticker: 'MET', sector: 'Financials' },
  { ticker: 'AFL', sector: 'Financials' },
  { ticker: 'USB', sector: 'Financials' },
  { ticker: 'PNC', sector: 'Financials' },
  { ticker: 'COF', sector: 'Financials' },
  { ticker: 'DFS', sector: 'Financials' },

  // Industrials
  { ticker: 'GE', sector: 'Industrials' },
  { ticker: 'CAT', sector: 'Industrials' },
  { ticker: 'HON', sector: 'Industrials' },
  { ticker: 'RTX', sector: 'Industrials' },
  { ticker: 'LMT', sector: 'Industrials' },
  { ticker: 'NOC', sector: 'Industrials' },
  { ticker: 'DE', sector: 'Industrials' },
  { ticker: 'UPS', sector: 'Industrials' },
  { ticker: 'FDX', sector: 'Industrials' },
  { ticker: 'MMM', sector: 'Industrials' },
  { ticker: 'ITW', sector: 'Industrials' },
  { ticker: 'EMR', sector: 'Industrials' },
  { ticker: 'GD', sector: 'Industrials' },
  { ticker: 'ETN', sector: 'Industrials' },
  { ticker: 'WM', sector: 'Industrials' },
  { ticker: 'RSG', sector: 'Industrials' },
  { ticker: 'PCAR', sector: 'Industrials' },
  { ticker: 'CMI', sector: 'Industrials' },
  { ticker: 'CSX', sector: 'Industrials' },
  { ticker: 'NSC', sector: 'Industrials' },

  // Energy
  { ticker: 'XOM', sector: 'Energy' },
  { ticker: 'CVX', sector: 'Energy' },
  { ticker: 'COP', sector: 'Energy' },
  { ticker: 'SLB', sector: 'Energy' },
  { ticker: 'EOG', sector: 'Energy' },
  { ticker: 'MPC', sector: 'Energy' },
  { ticker: 'VLO', sector: 'Energy' },
  { ticker: 'PSX', sector: 'Energy' },
  { ticker: 'OXY', sector: 'Energy' },
  { ticker: 'DVN', sector: 'Energy' },
  { ticker: 'HAL', sector: 'Energy' },
  { ticker: 'BKR', sector: 'Energy' },
  { ticker: 'FANG', sector: 'Energy' },
  { ticker: 'PXD', sector: 'Energy' },
  { ticker: 'APA', sector: 'Energy' },

  // Materials
  { ticker: 'LIN', sector: 'Materials' },
  { ticker: 'APD', sector: 'Materials' },
  { ticker: 'SHW', sector: 'Materials' },
  { ticker: 'ECL', sector: 'Materials' },
  { ticker: 'NEM', sector: 'Materials' },
  { ticker: 'FCX', sector: 'Materials' },
  { ticker: 'NUE', sector: 'Materials' },
  { ticker: 'VMC', sector: 'Materials' },
  { ticker: 'MLM', sector: 'Materials' },
  { ticker: 'PKG', sector: 'Materials' },
  { ticker: 'IP', sector: 'Materials' },
  { ticker: 'CF', sector: 'Materials' },
  { ticker: 'MOS', sector: 'Materials' },
  { ticker: 'BALL', sector: 'Materials' },
  { ticker: 'AVY', sector: 'Materials' },

  // Real Estate
  { ticker: 'AMT', sector: 'Real Estate' },
  { ticker: 'PLD', sector: 'Real Estate' },
  { ticker: 'CCI', sector: 'Real Estate' },
  { ticker: 'EQIX', sector: 'Real Estate' },
  { ticker: 'PSA', sector: 'Real Estate' },
  { ticker: 'O', sector: 'Real Estate' },
  { ticker: 'WELL', sector: 'Real Estate' },
  { ticker: 'DLR', sector: 'Real Estate' },
  { ticker: 'SPG', sector: 'Real Estate' },
  { ticker: 'EXR', sector: 'Real Estate' },
  { ticker: 'VTR', sector: 'Real Estate' },
  { ticker: 'EQR', sector: 'Real Estate' },
  { ticker: 'AVB', sector: 'Real Estate' },
  { ticker: 'ARE', sector: 'Real Estate' },
  { ticker: 'BXP', sector: 'Real Estate' },

  // Utilities
  { ticker: 'NEE', sector: 'Utilities' },
  { ticker: 'SO', sector: 'Utilities' },
  { ticker: 'DUK', sector: 'Utilities' },
  { ticker: 'AEP', sector: 'Utilities' },
  { ticker: 'D', sector: 'Utilities' },
  { ticker: 'EXC', sector: 'Utilities' },
  { ticker: 'SRE', sector: 'Utilities' },
  { ticker: 'XEL', sector: 'Utilities' },
  { ticker: 'PCG', sector: 'Utilities' },
  { ticker: 'WEC', sector: 'Utilities' },
  { ticker: 'ES', sector: 'Utilities' },
  { ticker: 'ETR', sector: 'Utilities' },
  { ticker: 'PPL', sector: 'Utilities' },
  { ticker: 'CMS', sector: 'Utilities' },
  { ticker: 'AES', sector: 'Utilities' },
]

export const SPY500_SECTORS = [
  'Information Technology',
  'Communication Services',
  'Consumer Discretionary',
  'Consumer Staples',
  'Health Care',
  'Financials',
  'Industrials',
  'Energy',
  'Materials',
  'Real Estate',
  'Utilities',
] as const

export type SpySector = typeof SPY500_SECTORS[number]
